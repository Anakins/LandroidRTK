...
class LandroidRTKScheduler {
    ...
    public static function evaluate($eqLogic) {
        ...
        $msg_parts = array(
            "✂️ {$eqLogic->getName()} va tondre la pelouse.",
    ...
    private static function notifyNotReady($eqLogic, $config, $state, $today, $reason) {
        ...
        if ($reason == 'spacing') {
            ...
            $msg = "{$eqLogic->getName()} ne tondra pas aujourd'hui car le temps était trop humide aujourd'hui ou que les conditions ne sont pas optimales($condition_label).";
        }
    ...
    private static function estimateNextMowCore($eqLogic, $config) {
        ...
        // 4ter) Batterie : pas de prédiction fiable possible non plus, on
